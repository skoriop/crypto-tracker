...
const MarketGraph = () => {
    ...
    useEffect(() => {
        axios.get(`https://api.coingecko.com/api/v3/coins/${coin}/market_chart?vs_currency=usd&days=${timeView}`)
        .then(res => {
            setData(res.data);
        })
    ...
    useEffect(() => {
        axios.get(`https://api.coingecko.com/api/v3/coins/${coin}?localization=false&tickers=false&market_data=false&community_data=false&developer_data=false&sparkline=false`)
        .then(res => {
            setAbout(res.data);
        })
        .catch(err => console.log(err))
    ...
    for (let i = 0; i < coinData.length; i++)
    {
        ...
        {
            newdate = date.toDateString().slice(3);
        ...
        else if (timeView === TimeOptions.P7D)
        {
            ...
            newdate = date.toDateString().slice(3, -5) + newdate[1].slice(0, -6);
        }
    ...
    return (
        <div className="market-view">
            <div className="coin-header">
                ...
                <h1 className="coin-header-text">{about?.name} ({about.symbol?.toUpperCase()})</h1>
            </div>
            <div className="graph-container">
                <div className="coin-graph">
                    <LineChart width={1000} height={500} data={coinData}>
                        <CartesianGrid stroke="#555" strokeDasharray="5 5" />
                        <XAxis dataKey="time" tick={{ transform: "translate(0, 5)" }} />
                        <YAxis type="number" tickCount={8} domain={["auto", "auto"]} tick={{ transform: "translate(-5, 0)" }} />
                        <Tooltip 
